// Teste que verifica se a clínica está sendo encontrada corretamente no webhook
...
async function testWebhookClinicSearch() {
  try {
    console.log('🧪 Verificando busca da clínica no webhook...');
    ...
    // 3. Buscar dados da clínica para obter o número do WhatsApp
    console.log('\n🔍 3. Buscando dados da clínica...');
    const { data: clinicDetails, error: clinicDetailsError } = await supabase
      .from('clinics')
      .select('whatsapp_phone, contextualization_json')
      .eq('id', clinicData.clinic_id)
      .single();
    ...
    console.log('✅ Dados da clínica obtidos:');
    console.log('  - WhatsApp:', clinicDetails.whatsapp_phone);
    console.log('  - Tem contextualização:', !!clinicDetails.contextualization_json);
    
    if (!clinicDetails.contextualization_json) {
      console.log('❌ PROBLEMA: Clínica não tem dados de contextualização');
      return;
    }
    
    // 4. Simular processamento de agendamento
    console.log('\n📝 4. Simulando processamento de agendamento...');
    
    // Simular o que o AppointmentConversationService faria
    const clinicContext = clinicDetails.contextualization_json;
    const specialties = clinicContext.clinica?.informacoes_basicas?.especialidades_secundarias || [];
    
    console.log('✅ Dados da clínica carregados:');
    console.log('  - Nome:', clinicContext.clinica?.informacoes_basicas?.nome);
    console.log('  - Especialidades:', specialties.length);
    
    // 5. Simular resposta de agendamento
    console.log('\n📤 5. Simulando resposta de agendamento...');
