-- ========================================
-- INSTALAÇÃO COMPLETA - TODOS OS SPRINTS (VERSÃO LIMPA)
-- ========================================
...
BEGIN
    ...
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'clinics') THEN
        RAISE EXCEPTION 'Tabela "clinics" não encontrada. Execute primeiro os scripts de setup básico.';
    END IF;
    
    -- Verificar se a tabela user_profiles existe (em vez de users)
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'user_profiles') THEN
        RAISE EXCEPTION 'Tabela "user_profiles" não encontrada. Execute primeiro os scripts de setup básico.';
    END IF;
