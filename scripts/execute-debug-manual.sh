...
# Script para execução manual dos comandos de debug na VPS
# Execute este script diretamente na VPS via SSH

echo "🔍 Debug Manual da Conexão WhatsApp"
echo "==================================="
...
# 1. Fazer backup
echo "📦 1. Fazendo backup do arquivo server.js..."
cp "$SERVER_FILE" "$BACKUP_FILE"
echo "✅ Backup criado: $BACKUP_FILE"

# 2. Adicionar logs de debug
echo ""
echo "🔧 2. Adicionando logs de debug..."

# Verificar se o endpoint existe
if ! grep -q "app.post.*generate-qr" "$SERVER_FILE"; then
    echo "❌ Endpoint /api/whatsapp/generate-qr não encontrado"
    exit 1
fi
...
RESPONSE=$(curl -s -X POST http://localhost:3001/api/whatsapp/generate-qr \
    ...
    -d '{"agentId": "test-debug-manual"}' 2>/dev/null || echo "ERRO")
...
# 5. Monitorar logs
echo ""
echo "📊 5. Monitorando logs por 60 segundos..."
