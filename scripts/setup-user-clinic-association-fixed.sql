-- Script corrigido para implementar associação de usuários a clínicas
...
-- 7. Limpar registros órfãos na user_profiles (usuários que não existem mais em auth.users)
DELETE FROM public.user_profiles 
WHERE user_id NOT IN (SELECT id FROM auth.users);

-- 8. Associar todos os usuários existentes à clínica principal
-- Primeiro, atualizar user_profiles com clinic_id (apenas para usuários válidos)
...
-- 9. Inserir associações na tabela clinic_users (apenas para usuários válidos)
INSERT INTO public.clinic_users (user_id, clinic_id, role, is_active)
SELECT 
  up.user_id,
  COALESCE(up.clinic_id, '00000000-0000-0000-0000-000000000001') as clinic_id,
  up.role,
  true as is_active
FROM public.user_profiles up
WHERE up.user_id IN (SELECT id FROM auth.users)  -- Verificar se usuário existe
AND NOT EXISTS (
  SELECT 1 FROM public.clinic_users cu 
  WHERE cu.user_id = up.user_id 
  AND cu.clinic_id = COALESCE(up.clinic_id, '00000000-0000-0000-0000-000000000001')
);

-- 10. Atualizar políticas RLS da tabela clinics para usar clinic_users
...
-- 11. Criar função para verificar se usuário é admin lify
...
-- 12. Criar função para verificar se usuário tem acesso global
...
SELECT 'Usuários válidos e suas clínicas:' as check_type, 
       up.email,
       up.role,
...
WHERE up.user_id IN (SELECT id FROM auth.users)  -- Apenas usuários válidos
...
-- 14. Verificar se há usuários órfãos
SELECT 'Usuários órfãos (não existem em auth.users):' as check_type,
