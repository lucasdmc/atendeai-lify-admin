...
export class ConversationMemoryService {
  ...
  static async resetLoopCount(phoneNumber: string): Promise<void> {
    const memory = await this.loadMemory(phoneNumber);
    memory.loopCount = 0;
    memory.frustrationLevel = Math.max(0, memory.frustrationLevel - 0.2);
  ...
  static async incrementLoopCount(phoneNumber: string): Promise<void> {
    const memory = await this.loadMemory(phoneNumber);
    memory.loopCount++;
    memory.frustrationLevel = Math.min(1, memory.frustrationLevel + 0.1);
  ...
  private static isCacheValid(memory: ConversationMemory): boolean {
    if (!memory.userProfile.lastInteraction) return false;
    
    const age = Date.now() - memory.userProfile.lastInteraction.getTime();
    return age < this.CACHE_TTL;
  }

  private static updateSentimentAnalysis(memory: ConversationMemory, message: string): void {
    // Palavras indicativas de frustração
    const frustrationKeywords = [
      'não funciona', 'problema', 'erro', 'demora', 'péssimo',
      'horrível', 'cansado', 'irritado', 'desisto', 'cancelar tudo'
    ];
    
    const lowerMessage = message.toLowerCase();
    const hasFrustration = frustrationKeywords.some(k => lowerMessage.includes(k));
    
    if (hasFrustration) {
      memory.frustrationLevel = Math.min(1, memory.frustrationLevel + 0.15);
    } else {
      // Reduzir frustração gradualmente
      memory.frustrationLevel = Math.max(0, memory.frustrationLevel - 0.05);
    }
  }

  private static updateTopics(memory: ConversationMemory, intent: Intent): void {
    // Adicionar tópico baseado na intenção
